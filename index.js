...
$(() => {
    ...
    $showMenuBtn.click(() => {
        ...
        $menuDiv.on("mouseleave", (event) => {
            setTimeout(() => {
                ...
                $menuDiv.off("mouseleave");
            }, 322);
        });
    });

    function resetMenuBtnsClass() {
        $('.menu-btn').removeClass("list-group-item-secondary")
    ...
    $('.menu-btn').on('click', (event) => {
        ...
        if ($(event.target).hasClass("menu-btn")) {
            $(event.target).addClass("list-group-item-secondary")
                .addClass("font-weight-bold")
                .addClass("border-dark");
        } else {
            $(event.target).parent().addClass("list-group-item-secondary")
                .addClass("font-weight-bold")
                .addClass("border-dark");
        }
    });
